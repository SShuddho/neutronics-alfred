...
r2 = 0.45
height = 60
number_of_assembly_mox1 = 57
number_of_assembly_mox2 = 114
number_of_pin_per_assembly = 127

mox1.volume = pi * ((r2**2) - (r1**2)) * height * number_of_assembly_mox1 * number_of_pin_per_assembly
mox2.volume = pi * ((r2**2) - (r1**2)) * height * number_of_assembly_mox2 * number_of_pin_per_assembly

volume_mox1 = mox1.volume
volume_mox2 = mox2.volume

fuel_volume_total = volume_mox1 + volume_mox2
...
# Create tallies that is necessary for unit normalization from OpenMC units (neutrons-cm/src) to conventional units (neutrons/cm^2-s)
...
# The number between statepoint. and .h5 is the number of batches
# Saving the stored data of statepoint file in the variable sp

sp = openmc.StatePoint(f'./statepoint.{number_of_batches}.h5')


# Fetching the values of flux
t = sp.get_tally(scores=['flux'])
flux = t.mean.ravel()           # [neutrons-cm/source]

print(flux)

# Calculating the Q Value for Fuel Content with Both the MOX1 and the MOX2

# Define Q-values in MeV for each isotope
Q_values_MeV = {
    'Pu238': 205.0,
    'Pu239': 207.1,
    'Pu240': 211.3,
...
# Mass fractions of plutonium isotopes
pu_mass_fractions = {
    'Pu238': 2.332 / 100,
    'Pu239': 56.873 / 100,
    'Pu240': 26.997 / 100,
...
# Compute Q-value for plutonium by summing weighted Q-values
Q_Pu_MeV = sum(pu_mass_fractions[iso] * Q_values_MeV[iso] for iso in pu_mass_fractions)
Q_Pu_J = Q_Pu_MeV * MeV_to_J

# MOX fuel fractions
MOX1_PuO2_frac = 0.217
MOX1_UO2_frac = 0.783
MOX2_PuO2_frac = 0.278
MOX2_UO2_frac = 0.722

# Compute Q-values for MOX1 and MOX2
Q_U235_J = Q_values_MeV['U235'] * MeV_to_J
Q_MOX1_J = MOX1_PuO2_frac * Q_Pu_J + MOX1_UO2_frac * Q_U235_J
Q_MOX2_J = MOX2_PuO2_frac * Q_Pu_J + MOX2_UO2_frac * Q_U235_J


# Effective Q-value for the whole MOX fuel
Q_mox_effective = (Q_MOX1_J * volume_mox1 + Q_MOX2_J * volume_mox2) / fuel_volume_total
...
V = fuel_volume_total                             # [cm^3]
P = 300e6                                         # [J/sec]
Q = Q_mox_effective                               # [J/fission]
nu = neu_fission / fiss_rate                      # [neutrons/fission]
k = neu_fission                                   # [neutrons/source]

normalization_factor = ((P * nu) / (V * Q * k))   # [src/sec-cm3]
normalized_flux = flux * normalization_factor     # [neutrons/cm^2-sec]

# Plot the figures with following params

dpi = 300
figsize = (10, 6)

# Plotting the neutron spectrum as flux per lethargy
fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
ax.step(energies[:-1], normalized_flux/np.diff(energies), color='darkblue', where='post')
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_xlabel('Energy [eV]')
ax.set_ylabel(r'Flux [(n/cm$^2$-s)/eV]')
ax.grid(True)

# If saving the image is needed
# plt.savefig("flux_per_lethargy_plot.png", dpi=dpi, bbox_inches='tight')

plt.show()
plt.close()

# Plotting the neutron spectrum in a loglog plot
fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
ax.loglog(energies[:-1], normalized_flux, '-', color='darkblue')
ax.set_xlabel('Energy [eV]')
ax.set_ylabel(r'Flux [n/cm$^2$-s]')
ax.grid(True)

# If saving the image is needed
# plt.savefig("flux_plot_loglog.png", dpi=dpi, bbox_inches='tight')

plt.show()
plt.close()

# Plotting the neutron spectrum in a semilog plot
fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
ax.plot(energies[:-1], normalized_flux, '-', color='darkblue', linewidth=2)
ax.set_xscale('log')
ax.set_xlabel('Energy [eV]')
ax.set_ylabel(r'Flux [n/cm$^2$-s]')
ax.grid(True)
...
# If saving the image is needed
# plt.savefig("flux_plot_semilog.png", dpi=dpi, bbox_inches='tight')
